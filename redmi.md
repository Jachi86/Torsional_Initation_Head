...
```text
(GJ/L) ? = M_CABLE + W d sin(?)
```

con la convenzione angolare:

- `? = 0°`: il peso `W` è sopra il punto `B`;
- `? = 90°`: il peso è a destra di `B`;
- `? = 180°`: il peso è sotto `B`.
...
- Grafico **Momenti vs ?** con:
  - `M_tube = (GJ/L) ?`;
  - `M_ext = M_CABLE + W d sin(?)`;
...
- `?s > 0`: equilibrio stabile;
- `?s < 0`: equilibrio instabile.
